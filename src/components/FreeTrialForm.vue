<template>
  <section
    id="free-trial"
    class="py-6 md:py-16 px-4 bg-[#b2d7d7] md:bg-transparent"
  >
    <div class="container mx-auto">
      <div class="flex flex-col items-center">
        <h2
          class="text-lg md:text-3xl font-bold text-[#003B39] md:text-primary-dark text-center mb-2 md:mb-4"
        >
          {{ formData.title }}
        </h2>
        <p
          class="text-xs md:text-base text-[#003B39] md:text-navy text-center mb-6 md:mb-8"
        >
          {{ formData.subtitle }}
        </p>
      </div>
      ...
      <form class="space-y-3 md:space-y-4" @submit.prevent="submitForm">
        ...
        <div class="flex items-start">
          ...
          <label
            for="promo-code"
            class="ml-2 text-xs md:text-sm text-[#003B39] md:text-navy"
          >
      ...
      <div
        class="mt-4 md:mt-8 text-[10px] md:text-xs text-[#003B39] md:text-navy-light space-y-1 md:space-y-2"
      >
        <p v-for="(disclaimer, index) in formData.disclaimers" :key="index">
          {{ disclaimer }}
        </p>
      </div>
    </div>
  ...
  formData: {
    ...
    default: () => ({
      ...
      checkboxes: {
        ...
        terms:
          "I have read the Terms and Conditions and the Privacy Policy and agree to them.",
      },
